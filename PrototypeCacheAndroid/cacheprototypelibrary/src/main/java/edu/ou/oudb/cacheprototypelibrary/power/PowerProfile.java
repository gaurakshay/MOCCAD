...
package edu.ou.oudb.cacheprototypelibrary.power;
...
import edu.ou.oudb.cacheprototypelibrary.R;
...
public class PowerProfile {
    ...
    public static final String POWER_CPU_VOLTAGE = "cpu.voltage";
    ...
    /**
     * power value read statically from power_profile_htc_one_m7.xml
     * NEED TO BE MODIFIED AND PROVIDED BY THE APPLICATION
    ...
    public double getCPUPower(double freq)
    {
    	Object dataSpeeds = sPowerMap.get(POWER_CPU_SPEEDS);
    	Object dataCurrents = sPowerMap.get(POWER_CPU_ACTIVE);
    	boolean found= false;
    	double current = -1;
    	
    	if(dataSpeeds instanceof Double[] && dataCurrents instanceof Double[])
    	{
    		Double [] frequencies = (Double []) dataSpeeds;
    		Double [] currents = (Double []) dataCurrents;
    		int flength = frequencies.length;
    		int clength = currents.length;
    		int iresult = 0;
    		
    		if (flength == clength)
    		{
	    		for(int i=0; i<flength && !found; ++i)
	    		{
    		...
    		if (found)
    		{
    			current = currents[iresult];
    		}
    	}
    	else if (dataSpeeds instanceof Double && dataCurrents instanceof Double)
    	{
    		current = (Double) dataCurrents;
    	}
